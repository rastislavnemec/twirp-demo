 ...
 */
final class StockServer implements RequestHandlerInterface
{
    const PATH_PREFIX = '/twirp/footshop.Stock/';
    ...
    public function handle(ServerRequestInterface $req): ResponseInterface
    {
        ...
        $ctx = Context::withPackageName($ctx, 'footshop');
        $ctx = Context::withServiceName($ctx, 'Stock');
        ...
        switch ($req->getUri()->getPath()) {
            case '/twirp/footshop.Stock/ReservationCreate':
                return $this->handleReservationCreate($ctx, $req);
    ...
    private function handleReservationCreate(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleReservationCreateJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleReservationCreateProtobuf($ctx, $req);
                break;
    ...
    private function handleReservationCreateJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'ReservationCreate');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Footshop\ReservationCreateRequest();
            $in->mergeFromJsonString((string)$req->getBody());

            $out = $this->svc->ReservationCreate($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling ReservationCreate. null responses are not supported'));
            }
    ...
    private function handleReservationCreateProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'ReservationCreate');
